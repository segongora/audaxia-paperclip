...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Datamatic Software", managedLabel: "Datamatic Software managed" };
    default:
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: "Skill created",
        body: `${skill.name} is now editable in the Datamatic Software workspace.`,
      });
    },
    onError: (error) => {
      pushToast({
